...
**目前版本**: v13.2 靈魂畫手手機版畫布修正

### 更新歷史

#### v13.2 (2026-02-10) - 靈魂畫手手機版畫布修正
- ✅ **手機橫放畫布溢出修復**：
  - 原問題：手機橫放時畫布接近正方形且超出畫面，需上下滑動才能看到完整畫布
  - 修正方案：改用高度為主要約束 (`h-[calc(100vh-140px)]`)，讓寬度自動根據 16:9 比例計算
  - 效果：橫放時畫布完全在視窗內，比例與電腦版一致 (16:9)

#### v13.1 (2026-01-27) - SpyGame 投票併發衝突修復
- ✅ **原子化投票更新 (v2.2)**：
  - 原問題：多位玩家同時按下「確認投票」時會發生寫入失敗或 Contention 錯誤
  - 根本原因：舊寫法使用 `{ ...votes, [uid]: targetId }` 整個物件覆寫，高併發時造成衝突
  - 解決方案：改用 Firestore **Dot Notation** 點記法 `updateDoc({ [`votes.${uid}`]: targetId })`
  - 效果：每位玩家只更新自己的欄位，Firebase 自動合併，完全避免 Contention
- ✅ **錯誤處理增強**：新增 try-catch 包覆，投票失敗時允許重試並顯示錯誤訊息
- ✅ **Debug Log**：新增投票成功/失敗日誌

#### v13.0 (2026-01-23) - 靈魂畫手功能調整
- ✅ **移除換題功能**：修正遊戲平衡性，移除「換一題」按鈕及相關邏輯。
- ✅ **新增筆刷粗細調整**：
  - 新增 `strokeWidth` 狀態與滑桿 (Slider) UI。
  - 支援 1px ~ 20px 筆刷粗細調整，提升繪圖體驗。
  - 橡皮擦粗細與筆刷同步。
